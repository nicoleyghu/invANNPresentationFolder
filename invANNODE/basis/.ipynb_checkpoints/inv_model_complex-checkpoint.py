...
def model(model_params,x):
        ...
        mp = np.abs(model_params)
        
        return np.array([[-2*mp[0]*(x[0]**2)-mp[2]*(x[1]*x[0])+3*mp[3]*x[3]**2+mp[4]*x[2]**2],
                                         [mp[0]*(x[0]**2) - 1.5*mp[1]*((x[1])**1.5)-mp[2]*(x[1]*x[0])],
                                         [mp[1]*((x[1])**1.5)-2*mp[4]*x[2]**2],
                                         [2*mp[2]*(x[1]*x[0])-2*mp[3]*x[3]**2]])
        """
...
model_params0 = np.array([1.,.1,13.,.4,5.])
#model_params0 = np.array([1.,1.])
bc0=np.array([0.4,0.1,0.2,0.3])
...
opt_init_nn, opt_update_nn, get_params_nn = optimizers.adam(1e-4, b1=0.9, b2=0.9999, eps=1e-100)#, b1=0.1, b2=0.999, eps=1e-10)
opt_init_model, opt_update_model, get_params_model = optimizers.adam(1e-4, b1=0.9, b2=0.9999, eps=1e-100)
