...
model_params0 = np.array([.9,.1,13.,.4,5.])
...
for j in range(num_eras):
    ...
    #batch = [_[np.arange(l)] for _ in data]
...
plt.figure(figsize=[10,10*2./3])
plt.plot(t, x,'o',ms=3,alpha=0.5)
plt.plot(t.flatten(),batched_state(nn_params, t),'-',lw=0.8,ms=2)
