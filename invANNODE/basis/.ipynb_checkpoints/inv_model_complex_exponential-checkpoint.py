...
def model(model_params,x):
    ...
    return np.array([[-2*mp[0]*mp[2]*(x[0]**2)-(mp[2])*(x[1]*x[0])+3*mp[3]*x[3]**2+mp[4]*x[2]**2],
                     [mp[0]*(x[0]**2) - 1.5*mp[1]*((x[1])**1.5)-mp[2]*(x[1]*x[0])],
                     [mp[1]*((x[1])**1.5)-2*mp[4]**2*x[2]**2],
                     [2*mp[2]*(x[1]*x[0])-2*np.exp(mp[3])*x[3]**2]])
    """
...
model_params0 = np.array([.9,.1,13.,.4,2.])
...
def loss(nn_params, model_params, batch):
  ...
  return err_data.mean()+0.001*err_model.mean()#*(1.+err_data.var())+np.sum([-np.nan_to_num(np.log(-i)) for i in model_params])#((pred_x[0]-bc)**2).mean()#+((pred_x.sum(axis=1)-1.)**2).mean()
